...
const run = async ({ material }: { material: boolean }) => {
  ...
  fs.writeFileSync(
    paths.resolvePath(material ? 'dist/index.less' : 'dist/mb.less'),
    "@import '../es/index.less';\n\n"
  );

  fs.writeFileSync(
    paths.resolvePath(material ? 'dist/index.css' : 'dist/mb.css'),
    result.css
  );
  ...
  fs.writeFileSync(
    paths.resolvePath(material ? 'dist/index.min.css' : 'dist/mb.min.css'),
    compress.styles
  );
