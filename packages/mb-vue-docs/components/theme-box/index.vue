          ...
          <Card class="theme-box-card">
            ...
            <template #actions>
              <Button
                ...
                type="text"
                size="mini"
                :href="`https://mb.design/themes/design/${item.themeId}`"
              >
                <template #icon>
                  <IconLink />
                </template>
      ...
      <template v-else>
        <Empty style="margin: 200px 0">
          <template #description>
            {{ t('themeBox.noResult') }}
            <Link :href="`https://mb.design/themes`">
...
import { IconSkin, IconLink } from '@mb-design/web-vue/es/icon';
...
const THEME_LINK_ID = 'mb-custom-theme';
...
export default defineComponent({
  ...
  setup() {
    ...
    const fetchThemeList = async (current: number, search: string) => {
      ...
      try {
        const data = await axios.get(
          `${apiBasename}/themes/api/open/themes/list?pageSize=6&currentPage=${current}&depLibrary=@mb-design/web-vue&keyword=${search}`
        );
    ...
    const addTheme = (_theme: ThemeData, notice: boolean) => {
      const url = `${_theme.unpkgHost}${_theme.packageName}/css/mb.css`;
      axios
        .get(url)
        .then(() => {
          if (theme.value) {
