<template>
  <mb-article v-bind="data">
    <div class="mb-vue-icon-header">
    ...
    <section v-for="data of icons" :key="data.type" class="mb-vue-icon-section">
      <h3 class="mb-vue-icon-section-title">{{ t(`icon.${data.type}`) }}</h3>
      <ul class="icon-list">
        <li
  ...
  setup() {
    ...
    const data = computed(() => {
      if (locale.value === 'zh-CN') {
        return {
          title: '图标 Icon',
          description: '这里有 Mb Design 内置的大量图标。',
          meta: {
            type: '组件',
            category: '通用',
          },
      ...
      return {
        title: 'Icon',
        description: 'There are a lot of icons built into Mb Design.',
        meta: {
          type: 'Component',
          category: 'Common',
        },
