...
const assertActiveAtIndex = (wrapper: any, index: number) => {
  const items = wrapper.findAll('.mb-carousel-indicator-item');
  const isActiveAtIndex = items[index]
    ?.classes()
    .includes('mb-carousel-indicator-item-active');
...
describe('Carousel', () => {
  ...
  test('Clicking arrow should be able to switch carousel', async () => {
    ...
    await nextTick();
    const nextIndicator = wrapper.find('.mb-carousel-arrow-right');
    assertActiveAtIndex(wrapper, 0);
  ...
  test('Clicking indicator should be able to switch carousel', async () => {
    ...
    await nextTick();
    const indicators = wrapper.findAll('.mb-carousel-indicator-item');
  ...
  test('Slider indicator type should work', async () => {
    ...
    await nextTick();
    expect(wrapper.find('.mb-carousel-indicator-slider').exists()).toBe(true);
    wrapper.find('.mb-carousel-indicator-slider').trigger('click');
