...
describe('Typography', () => {
  ...
  test('Paragraph should support copyable', async () => {
    ...
    const copyIconWrapper = wrapper.find('.mb-typography-operation-copy');
    expect(copyIconWrapper.exists()).toBe(true);
    await copyIconWrapper.trigger('click');
    expect(wrapper.find('.mb-typography-operation-copied').exists()).toBe(true);
  });
  ...
  test('Paragraph should support editable', async () => {
    ...
    const editIconWrapper = wrapper.find('.mb-typography-operation-edit');
    expect(editIconWrapper.exists()).toBe(true);
    await editIconWrapper.trigger('click');
    expect(wrapper.find('.mb-typography-edit-content').exists()).toBe(true);
  });
  ...
  test('Paragraph should support ellipsis', async () => {
    ...
    await sleep(200);
    const moreElement = wrapper.find('.mb-typography-operation-expand');
