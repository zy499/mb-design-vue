...
describe('Image', () => {
  ...
  test('Render image with error src', async () => {
    ...
    expect(wrapper.find('.mb-image-error').exists()).toBe(true);
  });
  ...
  test('Footer should work', async () => {
    ...
    expect(wrapper.find('.mb-image-footer-caption-title').text()).toBe(
      'My title'
    );
  });
  ...
  test('Preview should close on mask click', async () => {
    ...
    const maskWrapper = wrapper.find('.mb-image-preview-wrapper');
    await maskWrapper.trigger('click');
    expect(wrapper.find('.mb-image-preview-wrapper').exists()).toBe(false);
  });
  ...
  test('Preview fullscreen should work', async () => {
    ...
    const fullscreenAction = wrapper.findAll(
      '.mb-image-preview-toolbar-action'
    )[0];
    await fullscreenAction.trigger('click');
    expect(
      wrapper.find('.mb-image-preview-img-container').attributes('style')
  ...
  test('Preview rotate right should work', async () => {
    ...
    expect(wrapper.find('.mb-image-preview-img').attributes('style')).toContain(
      'rotate(90deg)'
    );
  });
  ...
  test('Preview zoom in should work', async () => {
    const wrapper = await getPreviewInstance();
    const zoomInAction = wrapper.findAll('.mb-image-preview-toolbar-action')[3];
  ...
  test('Should handle arrow click correctly', async () => {
    ...
    const rightIcon = wrapper.find('.mb-image-preview-arrow-right');
    await rightIcon.trigger('click');
    expect(wrapper.emitted('change')![0]).toContain(1);
    const leftIcon = wrapper.find('.mb-image-preview-arrow-left');
