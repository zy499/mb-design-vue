...
describe('Timeline', () => {
  test('Reverse should work', () => {
    ...
    expect(wrapper.classes()).toContain('mb-timeline-is-reverse');
  });
  ...
  test('Should work with timeline item', () => {
    ...
    expect(wrapper.findAll('.mb-timeline-item').length).toBe(2);
  });
  ...
  test('Dot type and dot color should work for timeline item', () => {
    ...
    const dot = wrapper.find('.mb-timeline-item-dot');
